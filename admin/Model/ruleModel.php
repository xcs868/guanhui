...
class ruleModel
{
    ...
    public function getData()
    {
        ...
        if( !empty($_POST['username']) ){
            $where = ' `rule_vch0` like "%'.$_POST['username'].'%"';
            $res = $this->db 	// 搜索为空
            ->field('*')
                ->where($where)
                ->order('`rule_inday` desc')
                ->limit($page.','.$rows)
                ->select();
        }else{
            $res = $this->db 	// 搜索不为空
    ...
    public function doAdd()	// 添加前台用户
    {
        // 1. 接收新增数据
        $data = array();
        $data['rule_vch0'] = $_POST['username'];
        $data['rule_vch1'] = $_POST['controller'];
        $data['rule_vch2'] = $_POST['methods'];
        $data['rule_int0'] = 1;
        $data['rule_inday'] = date('Y-m-d H:i:s',time());
        $data['rule_upday'] = date('Y-m-d H:i:s',time());
        $data['rule_dat0'] = date('Y-m-d H:i:s',time());
